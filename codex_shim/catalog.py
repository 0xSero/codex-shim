...
from .settings import FactoryModel, PROVIDER_NAME, chatgpt_passthrough_enabled, default_model_slug
...
def write_catalog(models: list[FactoryModel], path: Path, include_chatgpt: bool | None = None) -> Path:
    path.parent.mkdir(parents=True, exist_ok=True)
    if include_chatgpt is None:
        include_chatgpt = chatgpt_passthrough_enabled()
    entries = [chatgpt_passthrough_entry()] if include_chatgpt else []
...
def write_config(
    ...
    catalog_path: Path,
    port: int,
    include_chatgpt: bool | None = None,
) -> Path:
    path.parent.mkdir(parents=True, exist_ok=True)
    default_slug = default_model_slug(models, include_chatgpt)
